...
(function($) {
    ...
    $.jCal = function (target, opt){
      ...
      var twoMonthsMinWidth = $(target).data('two-months-min-width');
  
      if(twoMonthsMinWidth)
        twoMonthsMinWidth = parseInt(twoMonthsMinWidth);
      else
        twoMonthsMinWidth = 755;
  
      sm = ((parseInt($(window).width()) >= twoMonthsMinWidth)? 2 : 1);
      ...
      opt = $.extend({
        ...
        days:			    6, //default number of days user can select
        showMonths:		sm, //how many side-by-side months to show
        ...
        drawBack:     function () {return false;}, //callback function for month being drawn
        dayOffset:		0, //0=week start with sunday, 1=week starts with monday
        scrollSpeed:  0, //default .animate() speed used
    ...
    function drawCalControl (target, opt) {
      ...
      if ( opt.monthSelect )
        ...
        function (e) {
          ...
          // tratamento para não retornar os messes passados
          var mes = {
          ...
          nomedomes = mes[ $($('.jCalMo')[0]).find('.monthName').text() ];
          anodomes = $($('.jCalMo')[0]).find('.monthYear').text();
          diadomes = nomedomes + '/1' + '/' + anodomes;
          primeiromes = new Date(diadomes);
          if( primeiromes >= new Date ){
      ...
      function drawCal (target, opt) {
            ...
            for ( var d = offsetDayStart, dE = ( copt.fd + copt.ld + ( 7 - offsetDayEnd ) ); d < dE; d++)
              ...
              if(typeof onDrawCal == 'function') onDrawCal.call();
